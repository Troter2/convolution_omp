...
// The outer row-loop is parallelized with OpenMP: each row is independent
// (reads only from the read-only input buffer and kernel, no write conflicts).
//
//
...
//    kPtr = kernel;
    ...
    // start convolution
    #pragma omp parallel for private(j, m, n, rowMin, rowMax, colMin, colMax, sum) schedule(static)
...
//                            sum += *(inPtr - n) * *kPtr;
                            sum += in[(i + kCenterY - m) * dataSizeX + (j + kCenterX - n)] * kernel[m * kernelSizeX + n];
...
//            else *outPtr = 0;
            if(sum >= 0) out[i * dataSizeX + j] = (int)(sum + 0.5f);
            else         out[i * dataSizeX + j] = (int)(sum - 0.5f);
...
//        tconv = tconv + (tim.tv_sec+(tim.tv_usec/1000000.0) - start);
        tconv = tconv + (omp_get_wtime() - start);
